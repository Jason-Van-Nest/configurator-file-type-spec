...
Version 0.1.3 adds the `required` field to ports, enabling optional ports within a CIS standard. Optional ports may be present on one side of a connection plane and absent on the other; the configurator silently treats unmated optional ports as terminated at the manufacturing stage. This addition supports the common case where a single CIS standard governs both fully-equipped products (e.g., a kitchen pod with hot water) and reduced-feature products (e.g., a closet pod without hot water).
...
The version of the CIS specification this file conforms to. Parsers MUST reject files with a major version they do not support. Files declaring a `cis_format_version` of `0.1.2` or later use the `connection_signature` model (§6); files declaring `0.1.0` or `0.1.1` use the deprecated `gender` model. Files declaring `0.1.3` or later support the `required` field on ports (§6.1).
...
"ports": [
  ...
  {
    "port_id": "hot_water_supply",
    "side": "dwelling_unit_side",
    "position_in": { "x": 3.0, "y": 90.0 },
    "tolerance_in": { "x": 0.25, "y": 0.25 },
    "category": "utility",
    "utility_connection_ref": "hot_water_supply_utility",
    "direction": "ingress",
    "connection_signature": "pex_barb",
    "required": false,
    "description": "Hot water supply port. Optional \u2014 dry-utility-only modules may omit this port."
  },
  {
...
| `required` | boolean | No (default `true`) | When `false`, this port may be present on one side and absent on the other without invalidating the mating. See §6.5 for asymmetric-presence handling |
...
1. **Port correspondence**: each port on one side MUST correspond to exactly one port on the other side, based on position within tolerance — except when the optional-port rule (rule 4) applies
2. **Signature compatibility**: paired ports MUST have a registered mating per Appendix F. Same-to-same signatures must match exactly; gendered signatures must pair `_M` with `_F`
3. **Utility consistency**: if both ports reference a utility connection, the connections MUST match (same `utility_id`)
4. **Optional-port presence**: when a port has `required: false`, it may be present on one side and absent on the other without invalidating the mating. The configurator silently treats any unmated optional port as terminated at the manufacturing stage. The CTO file's `validation_state` SHOULD record an informational entry naming each unmated optional port for traceability — for example, "Hot water supply port (`hot_water_supply`) on dwelling-unit side is unmated; will be capped at manufacturing per CIS-INT-1220 v0.2.0 §6.5 rule 4."
5. **Category consistency**: paired ports MUST have the same `category`

A configuration in which any required-port pair fails rules 1–3 or 5 is invalid at manufacturing-ready status, though it MAY be allowed during design-time exploration as a warning (see §15.3). Optional-port presence asymmetries do not generate warnings or errors.

#### 6.5.1 Manufacturing Implications of Permissive Optional Ports

Permissive handling of optional ports at the configurator level requires corresponding handling downstream at the manufacturing stage. Each unmated optional port is physically present in the product (since the factory has already built the pod's internal plumbing/wiring) and must be terminated with an appropriate cap, plug, or seal. The CIS standard SHOULD recommend termination methods for each port signature, though the actual termination is a manufacturing concern, not a configurator concern.
...
### 9.6 Adding Optional Ports to an Existing CIS Standard

Adding a new port with `required: false` to an existing CIS standard is a backward-compatible MINOR bump of the standard's `standard_version`. Existing products that conform to an older minor version continue to validate correctly: their absent ports are treated as unmated optional ports per §6.5 rule 4. Adding a new port with `required: true` is a breaking MAJOR bump.
...
- Port-pair mating rules (§6.5) MUST be enforced during mating validation, including the optional-port presence rule (§6.5 rule 4)
...
Optional parameters: version (e.g., "0.1.3")
...
### Appendix C: Migration Path Between Versions

#### From v0.1.2 to v0.1.3

The only addition in v0.1.3 is the optional `required` field on ports. Migration is trivial:

1. Update `cis_format_version` to `"0.1.3"`
2. No port changes required — all ports default to `required: true`, matching v0.1.2 semantics
3. To take advantage of optional ports, add `"required": false` to the relevant port entries

Files conforming to v0.1.2 are forward-compatible with v0.1.3 parsers without modification.

#### From v0.1.x (v0.1.0 or v0.1.1) to v0.1.2 or later
...
6. Update `cis_format_version` to `"0.1.2"` or later
...
- §1.3: added "Required" terminology context (implicit from the new field)
- §6.1: added `required` field to port schema, default `true`
- §6.5: added rule 4 governing optional-port presence asymmetry; added §6.5.1 on manufacturing implications
- §9.6: added rule that adding `required: false` ports is a MINOR bump of the standard
- §11.4: updated to reference the new optional-port mating rule
- Added second example port (hot water) to §6.1 illustrating `required: false` usage

Summary of changes from v0.1.1 (carried forward):
